...
stats$overlap<-sapply(all_data, function(x){
  sum(unlist(lapply(split(x,x$allele), function(x){
    counter=0
    z=x[x$peplen==10, "MHC_peptide"]
    for (y in x[x$peplen==9, "MHC_peptide"]){
      counter<-counter+length(grep(y,z))
    }
    counter
  })))
})
...
if (length(stats[,1]) > 10){
  for (i in rownames(stats)[-1]){
    ...
    # boost by overlap of 9/10 mer
    boost <- boost + boost_ovlp * ( (stats[i, "overlap"] >= 1) * 0.3 +
                                    (stats[i, "overlap"] >= 2) * 0.25 +
                                    (stats[i, "overlap"] >= 3) * 0.20 +
                                    (stats[i, "overlap"] >= 4) * 0.15 +
                                    (stats[i, "overlap"] >= 5) * 0.10 )
...
write.table(file=paste(file_prefix, "_concise_results.tsv", sep="", collapse=""),
            stats[,c("gene", "mutation", "peptide", "mod_rank", "old_rank", "num_MHC", "num_pept",
                     "overlap", "min_pc", "TPM", "binding_MHCs")], col.names=F, row.names=F,
